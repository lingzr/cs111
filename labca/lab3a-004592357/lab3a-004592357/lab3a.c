...
void print_blocks_of_sigle_indriectBlock(void* indirect_block_Buffer,FILE* indirectStream,int BN){
	uint32_t* ptr4=indirect_block_Buffer;
	int entryNumber=0;
	while(*(ptr4)!=0){
		fprintf(indirectStream, "%x,%d,%x\n",BN,entryNumber,*(ptr4));
		ptr4++;
		entryNumber++;
	}
...
void print_blocks_of_double_indriectBlock(void* indirect_block_Buffer,FILE* indirectStream,int BN,int fd){
	uint32_t* ptr4=indirect_block_Buffer;
	int entryNumber=0;
	while(*(ptr4)!=0){
		fprintf(indirectStream, "%x,%d,%x\n",BN,entryNumber,*(ptr4));
		void* Buffer = malloc(s.blockSize);
		pread(fd,Buffer,s.blockSize,(*(ptr4))*s.blockSize);
		print_blocks_of_sigle_indriectBlock(Buffer,indirectStream,*(ptr4));
		ptr4++;
		entryNumber++;
	}
}
void print_blocks_of_triple_indriectBlock(void* indirect_block_Buffer,FILE* indirectStream,int BN,int fd){
	uint32_t* ptr4=indirect_block_Buffer;
	int entryNumber=0;
	while(*(ptr4)!=0){
		fprintf(indirectStream, "%x,%d,%x\n",BN,entryNumber,*(ptr4));
		void* Buffer = malloc(s.blockSize);
		pread(fd,Buffer,s.blockSize,(*(ptr4))*s.blockSize);
		print_blocks_of_double_indriectBlock(Buffer,indirectStream,*(ptr4),fd);
		ptr4++;
		entryNumber++;
	}	
}
...
int load_groupDes_block(group_des_t groupDes,int fd,int groupDesNumber,FILE* bitMapStream,
		FILE* inodeStream, FILE* directoryStream,FILE* indirectStream){
				...
				*/
				if (in.file_type=='d')
				{
					...
					for (k=0; k<in.nblocks; k++)
					{
						...
						print_directory_entry(directory_entry_Buffer, directoryStream, in.ninode,&entryNumber);
						//printf("%d\n", k);
					}
				}
				/*
				*	if the inode has more than 10, which means it contains indirect blocks
				*/
				if(in.nblocks>12){
					int k;
					for(k=12;k<in.nblocks;k++){
						void* indirect_block_Buffer = malloc(s.blockSize);
						switch (k){
							case 12:
								pread(fd,indirect_block_Buffer,s.blockSize,
									(in.block_ptr[k])*s.blockSize);
								print_blocks_of_sigle_indriectBlock(indirect_block_Buffer,indirectStream,in.block_ptr[k]);
							break;
							case 13:
								pread(fd,indirect_block_Buffer,s.blockSize,
									(in.block_ptr[k])*s.blockSize);
								print_blocks_of_double_indriectBlock(indirect_block_Buffer,indirectStream,in.block_ptr[k],fd);
							break;
							case 14:
								pread(fd,indirect_block_Buffer,s.blockSize,
									(in.block_ptr[k])*s.blockSize);
								print_blocks_of_triple_indriectBlock(indirect_block_Buffer,indirectStream,in.block_ptr[k],fd);
...
int print_directory_entry(void* directory_entry_Buffer, FILE* directoryStream, int block_num,int* entryNumber)
{
...
int main(){
	...
	for(i=0;i<nBlockDes;i++){
		load_groupDes_block(&gd,fd,i,bitMapStream,inodeStream,directoryStream,indirectStream);
